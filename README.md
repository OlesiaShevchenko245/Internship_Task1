...
### Причини 
1. Задача CPU-bound з малим обсягом роботи
Кожен запис обробляється дуже швидко через простий парсинг XML та підрахунок у мапі. Сама операція займає мікросекунди, тому поділ на потоки дає дуже малий виграш у часі.
2. Накладні витрати на потоки більші, ніж робота всередині них
Створення пулу потоків, розподіл файлів, керування чергами — все це займає частину часу. Коли завдання та набір даних настільки невеликі, накладні витрати переважають над продуктивністю.
3. Обмеження CPU
На MacBook Air є всього 8–10 апаратних ядер, тому запуск 8 потоків для “легкої” задачі не збільшує продуктивність — ядра і так не завантажені.
4. XML-парсинг відносно дорогий, але не достатньо важкий
Парсинг займає основну частину часу, але одного файлу вистачає, щоб навантажити CPU лише частково. Щоб побачити різницю, потрібні десятки великих файлів (250–500 МБ кожен).
